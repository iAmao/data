  ...
  return computed({
    ...
    set(key, value) {
      if (this instanceof Model) {
        return updateViaInternalModel(this, key, value);
      }

      return getRecordDataFor(this).setDirtyAttribute(key, value);
    },
  }).meta(meta);
}

function updateViaInternalModel(record, key, value) {
  if (DEBUG) {
    if (['_internalModel', 'recordData', 'currentState'].indexOf(key) !== -1) {
      throw new Error(
        `'${key}' is a reserved property name on instances of classes extending Model. Please choose a different property name for your attr on ${record.constructor.toString()}`
      );
    }
  }
